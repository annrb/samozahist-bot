...
const SHEET_URL =
"https://script.google.com/macros/s/AKfycbznxQ_n7ZfbmpQFfI3LtNT0S0tUH9fiBB4YddUv8YHa7n3-zvg-6vlifuCnUd9CIcvk/exec";
...
📦 Ваше замовлення підтверджено та буде відправлене найближчим часом.

💳 Решту суми ви сплатите при отриманні у відділенні Нової Пошти.
  ...
  return;
    ...
    await sendMessage(
      ...
      "🎁 Керування бонус-кодами:",
      {
        reply_markup: {
          keyboard: [
            [{ text: "➕ Створити бонус-код" }],
            [{ text: "📋 Список бонус-кодів" }],
            [{ text: "📣 Розіслати бонус-код" }],
            [{ text: "🗑 Деактивувати код" }],
            [{ text: "⬅️ Назад" }]
          ],
          resize_keyboard: true
        }
...
// Скидаємо режим очікування фото-відгуку,
// якщо користувач переходить до іншої дії
if (
  text === "🛒 Асортимент" ||
  text === "📝 Оформити замовлення" ||
  text === "💳 Оплата / доставка" ||
  ...
  const isMenuButton = [
    ...
    "1️⃣ Повна оплата",
    "2️⃣ Накладний платіж (передоплата 100 грн, решта на пошті)",
...
💳 Передоплата — 100 грн.
📦 Решта суми — при отриманні у відділенні Нової Пошти.
...
if (
  ...
  text === "2️⃣ Накладний платіж (передоплата 100 грн, решта на пошті)"
) {
...
Для підтвердження замовлення необхідно внести передоплату 100 грн.
